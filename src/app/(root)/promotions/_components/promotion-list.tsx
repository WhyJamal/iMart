...
const countdown = (ms: number) => {
  ...
  return days > 0 ? `${days}д ${hours}ч` : hours > 0 ? `${hours}ч ${mins}м` : `${mins}м`;
};
...
function PromotionCard({ promotion: p }: { promotion: IPromotion }) {
  ...
  return <div className="rounded-2xl border bg-card p-5 shadow-sm">
    <div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h3 className="font-semibold">{p.name}</h3><Badge variant={expired?"secondary":"default"}>{p.discountPercent}%</Badge></div><p className="text-xs text-muted-foreground mt-1">{p.pointName} · {p.warehouseName} · {p.warehouseCellName}</p></div><Button variant="ghost" size="icon" onClick={remove} disabled={pending}><Trash2 className="w-4 h-4 text-destructive"/></Button></div>
    <div className="mt-4 flex flex-wrap gap-2">{p.items.map(i=><Badge key={i.id} variant="outline">{i.productName}</Badge>)}</div>
    <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground"><span className="flex items-center gap-1"><Clock3 className="w-3.5 h-3.5"/> до {moneyDate(p.endsAt)}</span><span className={expired?"text-muted-foreground":"font-semibold text-emerald-600"}>{countdown(remaining)}</span></div>
