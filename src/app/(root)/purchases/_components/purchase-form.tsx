...
interface LineItem
  extends Omit<PurchaseItemInput, "qty" | "unitCost"> {
  _key: string;
  qty: string;
  unitCost: string;
}
...
export function PurchaseForm({
  ...
  const warehousesForPoint = warehouses.filter(
    (w) => w.pointId === pointId
  );
  ...
  const handleSubmit = () => {
    ...
    if (!pointId) {
      toast.error(t("selectPointError"));
      return;
    }

    if (!contragentId) {
      toast.error(t("selectContragentError"));
      return;
    }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-4 border-b flex items-center justify-between">
        <h2 className="text-base font-semibold">
          {initialData
            ? t("editPurchaseReceipt")
            : t("newPurchaseReceipt")}
        </h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
            <Label>{t("contragent")}</Label>

            <Select
              value={contragentId}
              onValueChange={setContragentId}
            >
        ...
        <div className="space-y-1.5">
          ...
          <Select
            value={paymentMethod}
            onValueChange={(v) =>
              setPaymentMethod(v as CashMethod)
            }
          >
            <SelectTrigger>
              <SelectValue />
