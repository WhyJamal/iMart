...
import type { IWorkScheduleDetail, IScheduleDay } from "@/types/work-schedule.types";
...
export function ScheduleTable({ schedule, canManage }: Props) {
  const router = useRouter();
  const [editingKey, setEditingKey] = useState<string | null>(null);

  const dayMap = useMemo(() => {
    const map = new Map<string, IScheduleDay>();
    for (const d of schedule.days) map.set(d.date, d);
    return map;
  }, [schedule.days]);

  const { mutate: fill, isPending: isFilling } = useFillWorkSchedule(() =>
    router.refresh()
  );
  const { mutate: setDay, isPending: isSaving } = useSetScheduleDay(() => {
    router.refresh();
    setEditingKey(null);
  });
  ...
  const monthTotals = useMemo(() => {
    return Array.from({ length: 12 }, (_, m) => {
      let hours = 0;
      let workDays = 0;
      for (let d = 1; d <= daysInMonth(schedule.year, m); d++) {
        const key = `${schedule.year}-${String(m + 1).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
  ...
  return (
    <div className="space-y-4">
      {canManage && (
        <Button onClick={() => fill(schedule.id)} disabled={isFilling}>
              ...
              return (
                <tr key={m} className="border-t">
                  ...
                  {Array.from({ length: 31 }, (_, i) => {
                    ...
                    if (dayNum > dim) {
                      return (
                        <td key={i} className="px-1.5 py-1.5 text-center text-muted-foreground">
                          —
                        </td>
                      );
                    }
                    const dateKey = `${schedule.year}-${String(m + 1).padStart(2, "0")}-${String(dayNum).padStart(2, "0")}`;
      ...
      <p className="text-xs text-muted-foreground">
        Qizil katak — bayram/dam kuni (Ish kalendaridan). Katak ustiga
        bosib qo'lda tuzatish mumkin — tuzatilgan kunlar keyingi
        "Заполнить"da qayta yozilmaydi.
      </p>
    </div>
  );
}
