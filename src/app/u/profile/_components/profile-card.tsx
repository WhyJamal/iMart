...
import { Camera, Mail, Building2, LogOut } from "lucide-react";
  ...
  return (
    <div className="rounded-[18px] border border-[#d2d2d7] bg-white overflow-hidden">
      ...
      <div className="px-6 mt-20">
        ...
        {user.organization && (
          <p className="text-xl text-[#86868b]">{user.organization.name}</p>
        )}
      </div>
      ...
      <div className="mt-5 px-6 pb-2 space-y-3">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[#1d1d1f] truncate">
            {user.organization?.name ?? "Biriktirilmagan"}
          </span>
        </div>
        <div className="flex items-center gap-2.5">
          <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0 ml-0.75" />
          <span className="text-[#1d1d1f]">Faol</span>
        </div>
      </div>
